...
biggest_challenges = st.multiselect(
    "What are the biggest challenges you face when trying to assure digital twins for adherence to trustworthy and ethical principles?",
    ["Lack of resources", "Lack of skills", "Lack of awareness of available assurance techniques",
     ...
     "Lack of regulatory clarity as to which techniques will be adequate to be regulatory compliant",
     "Lack of internal demand", "Lack of external demand",
     "Concern around international interoperability",
     "Lack of mechanisms (e.g., certification) for external recognition for using AI assurance techniques",
     "Other"],
    key='4_1'
)
if "Other" in biggest_challenges:
    other_challenge = st.text_area("Please explain the other biggest challenges:", key='4_1_other')

# Question 4.2
st.subheader("4.2 Type of Support")
type_of_support = st.multiselect(
    "What type of support might help you implement digital twin assurance practices and operationalise to ethical principles?",
...
if len(type_of_support) > 1:
    st.subheader("4.2b Rank the Impact of Support Types from Top (High Impact) to Bottom (Low Impact)")
...
ethical_issues = st.text_area(
    "In your experience, what ethical issues or risks are unique or amplified in digital twin technology?",
    key='4_3'
)
...
if st.button("Submit & See Results"):
    ...
    if conn:
        ...
        # Update the data dictionary to match the questions from the "Needs and Challenges" section
        data = {
            "biggest_challenges": ', '.join(biggest_challenges),  # Joining list into a string for multiselect
            "type_of_support": ', '.join(type_of_support),  # Joining list into a string for multiselect
            "sorted_support_types": ', '.join(sorted_items) if type_of_support else None,  # Conditional based on type_of_support response
